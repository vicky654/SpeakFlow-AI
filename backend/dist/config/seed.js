...
async function seedInitialData() {
    try {
        ...
        if (users.length === 0) {
            ...
            const adminPass = await bcryptjs_1.default.hash('adminpassword', salt);
            const studentPass = await bcryptjs_1.default.hash('studentpassword', salt);
            // Seed Admin
            await dbService_1.dbService.users.create({
        ...
        if (vocabs.length === 0) {
            ...
            for (const item of vocabList) {
                await dbService_1.dbService.vocab.create(item);
            }
            console.log(`✅ Seeded ${vocabList.length} vocabulary words.`);
        }
        // 3. Seed Grammar Lessons if empty
        const grammarLessons = await dbService_1.dbService.lessons.getByType('grammar');
...
`,
            ...
            for (const item of lessons) {
                await dbService_1.dbService.lessons.create(item);
            }
            console.log(`✅ Seeded ${lessons.length} grammar lessons.`);
        }
        // 4. Seed Reading Lessons if empty
        const readingLessons = await dbService_1.dbService.lessons.getByType('reading');
...
`,
        ...
        const listeningLessons = await dbService_1.dbService.lessons.getByType('listening');
...
`,
        ...
        const interviewLessons = await dbService_1.dbService.lessons.getByType('interview');
