...
router.get('/lessons', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  if (!type || !['grammar', 'reading', 'listening', 'interview'].includes(type as string)) {
    return res.status(400).json({ message: 'Valid type query param is required.' });
  }

  try {
    const lessons = await dbService.lessons.getByType(type as string);
    res.json(lessons);
  } catch (error) {
...
// GET LESSON BY ID
router.get('/lessons/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const lesson = await dbService.lessons.findById(req.params.id);
    if (!lesson) {
...
// SUBMIT QUIZ
router.post('/lessons/:id/quiz', authenticateToken, async (req: AuthRequest, res: Response) => {
  const { answers } = req.body; // e.g. { "q1": "Answer A", "q2": "Answer B" }
  const userId = req.user!.id;
  const lessonId = req.params.id;

  try {
    const lesson = await dbService.lessons.findById(lessonId);
    if (!lesson) {
      return res.status(404).json({ message: 'Lesson not found.' });
    }
...
router.post('/practice/time', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (skill === 'speaking') speakingTime += duration;
    if (skill === 'listening') listeningTime += duration;
    if (skill === 'reading') readingTime += duration;
...
router.post('/practice/writing', authenticateToken, async (req: AuthRequest, res: Response) => {
...
router.post('/practice/speaking/evaluate', authenticateToken, async (req: AuthRequest, res: Response) => {
